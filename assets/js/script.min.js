const $=(e,t=document)=>t.querySelector(e),$$=(e,t=document)=>t.querySelectorAll(e);Node.prototype.on=function(e,t){return this.addEventListener(e,t)};var get={},set={},actions={},triggers={},state={database:null,table:null,databases_order:[],databases:{},tables:[]};function tabClassActive(){return["bg-navy-100","text-navy-900","border-navy-300"]}function tabClassInactive(){return["hover:bg-gray-100","bg-gray-50","bg-gradient-to-b","from-white","to-gray-100","border-gray-300"]}function hollowClassActive(){return["bg-navy-100","text-navy-900","border","border-navy-300","rounded"]}function hollowClassInactive(){return["hover:bg-grayExtra","border","border-transparent"]}function eventCellKeydown(){window.addEventListener("keydown",t=>{var e=$(`pane-main[db="${state.database}"][tb="${state.table}"]`);const s=$('cell-ring[state="active"]',e);if(s){let e=s.closest("table-cell");if(e)switch(t.key){case"ArrowLeft":case"ArrowRight":case"ArrowDown":case"ArrowUp":e.handleStep(t.key);break;case"Tab":e.handleCellTab(t);break;case"Enter":e.handleCellEdit()}}})}function handleCellClose(){isCellState("edit")&&(dom.current.cell_ring.setAttribute("state","active"),resetEdit())}class cell{static setPreview(e,t){$("preview-value",t.closest("table-cell")).innerHTML=e}static setTempValue(e,t){var s=table.get(t);console.log(s);const a=temp[row.getType(t)][s].data;a[row.getIndex(t)][cell.getColumn(t)]=e}static getTempValue(e){var t=table.get(e);return temp[row.getType(e)][t].data[row.getIndex(e)][cell.getColumn(e)]}static getValue(e){var t=e.closest("table-cell"),s=e.closest("pane-main"),a=s.db,e=s.tb,s=t.col,t=t.index;return get.tb.value(a,e,s,t)}static getColumn(e){return e.closest("table-cell").getAttribute("column")}static isNullable(){return"true"==obj.closest("table-cell").getAttribute("nullable")}static isNull(){return"true"==obj.closest("table-cell").getAttribute("is_null")}}function leaveEdit(e){in_field=!0,fieldClose(e)}function fieldClose(e){e=e.closest("table-cell");$('cell-ring[state="edit"]',e)&&($("cell-ring",e).setAttribute("state","active"),$("cell-edit",e).hidden=!0)}function updatePreview(e,t){$("preview-value",t.closest("table-cell")).innerHTML=e}function updateNull(e){const t=$("preview",e.closest("table-cell")),s=$("preview-value",e.closest("table-cell"));s.hidden=!0,t.setAttribute("active","true");const a=$("preview-null",e.closest("table-cell"));a.hidden=!1}function setCellFieldState(e,t){t.querySelector("cell-edit").setAttribute("active",e)}function setCellPreviewState(e,t){t.querySelector("cell-preview").setAttribute("active",e)}function cellActiveIndex(e){return[...e.parentElement.children].indexOf(e)+1}function isCellActive(){return dom.current&&"active"==dom.current.cell_ring.getAttribute("state")}function isCellState(e="active"){return dom.current&&dom.current.cell_ring.getAttribute("state")==e}function setCellActiveToEl(e,t="active"){e.setAttribute("state",t)}function isEmpty(e){return 0===Object.keys(e).length}function error(e){}function success(e){}function spreadDomCell(e){return current={table_cell:e,cell_ring:e.querySelector("cell-ring"),cell_edit:e.querySelector("cell-edit"),cell_preview:e.querySelector("cell-preview")}}function resetStore(){dom.current=null,dom.left=null,dom.right=null,dom.up=null,dom.down=null}function storeDomCell(e){dom.current=spreadDomCell(e),dom.left=spreadDomCell(getDomCellLeft()),dom.right=spreadDomCell(getDomCellRight()),dom.down=spreadDomCell(getDomCellDown()),dom.up=spreadDomCell(getDomCellUp())}class row{static isNew(e){return!!e.closest(".row-new")}static getType(e){return row.isNew(e)?"insert":"update"}static getIndex(e){return e.closest("table-row").dataset.index}}class tab{static onMiddleClick(t){t.on("mouseup",e=>{1===e.button&&tab.closeHandler(t)})}}class table{static get(e){let t=e;return"PANE-MAIN"!=e.tagName&&(t=e.closest("pane-main")),`${t.getAttribute("database")} ${t.getAttribute("table")}`}}actions.database={},actions.databases={},actions.databases.load=()=>{axios.get("server/php/queries/databases.php").then(e=>{200===e.status&&(set.database.order(e.data),set.database.items(e.data))})},actions.database.toggle=e=>{set.database.toggleState(e)},actions.table={},actions.tables={},actions.table.load=(t,s)=>{isEmpty(get.tb.items(t,s))?axios.get(`server/php/queries/data.php?database=${t}&table=${s}`).then(e=>{200===e.status&&(actions.table.activate(t,s),set.table.item(e.data,t,s))}):actions.table.activate(t,s)},actions.tables.load=t=>{axios.get(`server/php/queries/tables.php?database=${t}`).then(e=>{200===e.status&&(set.table.order(e.data,t),set.table.items(e.data,t))})},actions.table.activate=(e,t)=>{set.table.name(e,t)},actions.table.closeTab=(e,t)=>{set.table.item({},e,t),set.table.name(null,null)},get.db={},get.tb={},get.col={},get.tb.order=()=>state.databases[state.database].tables_order,get.tb.items=(e=state.database,t=state.table)=>state.databases[e].table_items[t],get.tb.value=(e,t,s,a)=>{return get.tb.items(e,t).rows[a][s]},get.col.data=(e,t,s)=>{return get.tb.items(e,t).cols[s]},set.database={},set.database.order=e=>{state.databases_order=e},set.database.items=e=>{state.databases={},e.forEach(e=>{state.databases[e]={open:!1,table_order:[],table_items:{},table_pending_update:{}}}),triggers.db.load()},set.database.toggleState=e=>{state.databases[e].open=!state.databases[e].open,triggers.db.toggle(e)},set.table={},set.pending={},set.table.order=(e,t)=>{state.databases[t].table_order=e},set.table.name=(e,t)=>{var s=state.database,a=state.table;state.database=e,state.table=t,state.table?triggers.tb.activate():triggers.tb.closeTab(s,a)},set.table.item=(e,t,s)=>{state.databases[t].table_items[s]=e,triggers.tb.data()},set.pending.update=(e,t)=>{var s=t.closest("pane-main");const a=t.closest("table-cell");var i=s.db,l=s.tb,n=a.getAttribute("col"),c=a.getAttribute("row"),s=a.getAttribute("index");const r=state?.databases[i]?.table_items[l];s=get.tb.value(i,l,n,s);r?.pending_updates||(r.pending_updates={}),r?.pending_updates[c]||(r.pending_updates[c]={}),s!==e?(r.pending_updates[c][n]=e,t.closest("table-cell").setAttribute("state","changed")):(delete r.pending_updates[c][n],r?.pending_updates[c]||delete r.pending_updates[c],r?.pending_updates||delete r.pending_updates,t.closest("table-cell").removeAttribute("state")),console.log(s),console.log(e),console.log(r.pending_updates)},set.table.items=(e,t)=>{e.forEach(e=>{state.databases[t].table_items[e]={}}),triggers.tb.items(t)};var test=[];test[3]="Hello",console.log(test),triggers.db={},triggers.db.load=()=>{$("db-list").databasesPopulate()},triggers.db.toggle=e=>{triggers.db.open(e),triggers.db.close(e)},triggers.db.open=e=>{state.databases[e].open&&($("db-list").arrowUp(e),$("db-list").showElement("tb-section",e),$("db-list").groupEmpty(e)&&actions.tables.load(e))},triggers.db.close=e=>{state.databases[e].open||($("db-list").arrowDown(e),$("db-list").hideElement("tb-section",e))},triggers.tb={},triggers.tb.activate=()=>{$("db-list").deactivateTb(),$("db-list").activateTb(),$("tab-items").activate()},triggers.tb.closeTab=(e,t)=>{$("db-list").deactivateTb(),$("tab-items").close(e,t)},triggers.tb.items=e=>{$("db-list").tablesPopulate(e),$("db-list").hideElement("tb-loading",e)},triggers.tb.data=()=>{$("main-x").deactivatePanes(),$("main-x").addPane()};class ActionbarRefresh extends HTMLElement{constructor(){super()}static get observedAttributes(){}connectedCallback(){this.innerHTML=`
      <div data-action>
    ...
    `}deactivate(){$$("actions-tab",this).forEach(e=>{e.deactivate()})}}customElements.define("actions-tabs",ActionsTabs);class ActionsX extends HTMLElement{constructor(){super()}connectedCallback(){this.classList.add("flex","flex-col","gap-4"),this.innerHTML=`      
      ...
      </div>`,this.onClick()}onClick(){$("button",this).addEventListener("click",()=>{$("actions-panes > *:not([hidden])",this).hidden=!0,$("actions-panes",this).removeAttribute("active"),$('actions-tab[active="true"]',this).deactivate()})}}customElements.define("pane-close",PaneClose);class ColumnsItem extends HTMLElement{constructor(){super()}static get observedAttributes(){return["checked"]}connectedCallback(){var e=this.getAttribute("name"),t=this.getAttribute("checked");this.classList.add("flex"),this.innerHTML=this.template(e,t),this.onClick()}template(e,t){return`
    ...
    `}attributeChangedCallback(e,t,s){t!==s&&"active"==e&&("true"==s?this.thisActivate():this.thisDeactivate())}thisActivate(){this.classList.remove("hidden")}thisDeactivate(){this.classList.add("hidden")}activate(){this.setAttribute("active","true")}deactivate(){this.removeAttribute("active")}}customElements.define("panes-x",PanesX);class DeleteX extends HTMLElement{constructor(){super()}static get observedAttributes(){return["active"]}connectedCallback(){this.classList.add("btn","btn-default"),this.innerHTML=`
      <img-svg src="remixicon/delete-bin-line.svg" classes="w-5 h-5"></img-svg>
      Delete checked
    `,this.onClick()}onClick(){this.addEventListener("click",()=>{$$(".row-new[active]").forEach(e=>{e.remove()}),this.delete()})}async delete(){try{return(await axios.post("http://localhost/tools/squares/server/php/queries/delete.php?database=asda&table=asda",{ids:[1,2,3]})).data}catch(e){console.error(e)}}}customElements.define("delete-x",DeleteX);class ExecuteX extends HTMLElement{constructor(){super()}static get observedAttributes(){return["active"]}connectedCallback(){this.classList.add("btn","btn-primary"),this.innerHTML=`
      <img-svg src="remixicon/flashlight-fill.svg" classes="w-5 h-5"></img-svg>
      Execute
    `}onClick(){this.addEventListener("click",()=>{this.run()})}async run(){try{return(await axios.post("http://localhost/tools/squares/server/php/queries/delete.php?database=asda&table=asda",{ids:[1,2,3]})).data}catch(e){console.error(e)}}}customElements.define("execute-x",ExecuteX);class PaginationX extends HTMLElement{constructor(){super()}connectedCallback(){this.innerHTML=`
    ...
    `,this.onCancel()}onCancel(){this.querySelector('button-item[style="ghost"]').addEventListener("click",()=>{$("modal-box").deactivate()})}}customElements.define("modal-logout",ModalLogout);class TableCell extends HTMLElement{constructor(){super()}connectedCallback(){this.db=this.closest("pane-main").getAttribute("db"),this.tb=this.closest("pane-main").getAttribute("tb"),this.col=this.getAttribute("col"),this.row=this.getAttribute("row"),this.index=this.getAttribute("index"),this.data=get.tb.items(this.db,this.tb),this.value=get.tb.value(this.db,this.tb,this.col,this.index),this.col_data=get.col.data(this.db,this.tb,this.col),this.innerHTML=`
      <cell-ring></cell-ring>
    ...
    `,this.xEdges(),this.setPreviewValue(),this.setPreviewNull(),this.onClickRing(),this.onDblclickRing()}setPreviewValue(){$("preview-value",this).innerText=this.value}setPreviewNull(){$("preview-null",this).hidden=null!==this.value,$("preview-value",this).hidden=null===this.value}onClickRing(){$("cell-ring",this).on("click",()=>{this.handleCellActive(this)})}onDblclickRing(){$("cell-ring",this).on("dblclick",e=>{this.handleCellEdit()})}onClickNull(){$("cell-edit label:first-child input",this).on("change",e=>{e=e.currentTarget.checked;$("preview-null",this).hidden=!e,$("preview-value",this).hidden=e})}handleStep(e){switch(e){case"ArrowLeft":this.stepLeft();break;case"ArrowRight":this.stepRight();break;case"ArrowUp":this.stepUp();break;case"ArrowDown":this.stepDown()}}stepLeft(){var e=this.previousElementSibling;e&&"TABLE-CELL"===e.tagName&&this.handleCellActive(e)}stepRight(){var e=this.nextElementSibling;e&&"TABLE-CELL"===e.tagName&&this.handleCellActive(e)}stepDown(){var e=cellActiveIndex(this),t=this.parentElement.nextElementSibling;t&&"TABLE-ROW"===t.tagName&&this.handleCellActive($(`table-cell:nth-child(${e})`,t))}stepUp(){var e=cellActiveIndex(this),t=this.parentElement.previousElementSibling;t&&"TABLE-ROW"===t.tagName&&this.handleCellActive($(`table-cell:nth-child(${e})`,t))}handleCellTab(e){e.preventDefault(),this.handleStep(e.shiftKey?"ArrowLeft":"ArrowRight")}handleCellEdit(){in_field?in_field=!1:($("cell-ring",this).setAttribute("state","edit"),this.populateEdit())}handleCellActive(e){"active"!=$("cell-ring",e).getAttribute("state")&&(this.closest("pane-main").deactivateCells(),$("cell-ring",e).setAttribute("state","active"))}isNullable(){return"YES"==this.col_data.meta.Null}populateEdit(){let e="";this.isNullable()&&(e=`
    ...
    `}}customElements.define("topbar-wrap",TopbarWrap);class FieldText extends HTMLElement{constructor(){super()}static get observedAttributes(){}connectedCallback(){var e=cell.getValue(this);console.log(state),this.innerHTML=`
    ...
    `,this.onKeyup(),this.onEnter(),this.onEscape(),updatePreview($("input",this).value,this),$("input",this).focus(),$("input",this).select()}onKeyup(){$("input",this).addEventListener("keyup",e=>{e=e.currentTarget.value;set.pending.update(e,this),updatePreview(e,this)})}onEnter(){$("input",this).addEventListener("keydown",e=>{"Enter"===e.key&&(e.preventDefault(),leaveEdit(e.currentTarget))})}onEscape(){window.addEventListener("keydown",e=>{var t;"Escape"===e.key&&(t=$(`pane-main[db=${state.database}][tb=${state.table}]`),(t=$('cell-ring[state="edit"]',t))&&(e.preventDefault(),fieldClose(t)))})}attributeChangedCallback(e,t,s){}}customElements.define("field-text",FieldText);